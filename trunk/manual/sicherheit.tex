...
oder Operative-Rechnern auszuführen. Mit der folgenden  Massnahme ist es
jedoch anderen Benutzern nicht möglich, schädlichen Code auszuführen. Dies
gilt natürlich nur dann, wenn die policy-Dateien und die Security-Manager
...
nötigsten Rechte auf den ausführenden Computern erhalten, gibt es die
policy-Dateien zur Konfiguration der Security-Manager. Dieses Konzept bietet
den Vorteil, dass jeder, der ein Java-Programm ausführt, in Klartext
(zumindest beinahe) die Rechte einsehen und vorgeben kann, die das
Java-Programm hat, indem er direkt die policy-Dateien anschaut bzw.\ ändert.
Welche policy-Dateien im Einzelnen zu verwenden sind, findet sich bei den
Kapiteln zu den jeweiligen Programmteilen.

Wichtige Einschränkungen in den policy-Dateien sind:

\begin{itemize}
\item Kein Zugriff auf das Dateisystem (außer durch den Problem-Übermittler).
...
Die weiteren Einstellungen können direkt aus den policy-Dateien abgelesen
werden. Die oben genannten Einschränkungen können natürlich aufgehoben
werden, falls dies in einer Anwendungssituation notwendig ist. Es ist
prinzipiell auch möglich, nur gewissen signierten Java-Klassen weitere
...
dass man in der policy-Datei des Dispatchers nur bestimmte Rechner als
Operatives erlaubt, indem man zu anderen Rechnern die
