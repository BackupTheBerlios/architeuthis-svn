...
\item  Die Teilprobleme müssen das Interface \texttt{CommunicationPartialProblem}
       ...
       den gemeinsamen Speicher benutzen, können eine \texttt{RemoteException}
...
\item  Aufeinanderfolgende Lesezugriffe zweier Teilprobleme ohne
       ...
       d.h.\ ohne daß während des Schreibens in die dezentralen Speicher ein
...
\item  Aufeinanderfolgende Lesezugriffe zweier Teilprobleme ohne
       ...
       erfolgen. Schreibzugriffe werden dann anschließend (d.h.\ nebenläufig
...
Es ist jedoch zu beachten, daß die \emph{Reihenfolge} der Lese- und
Schreibzugriffe auf den gemeinsamen Speichers durch die Teilprobleme
untereinander in allen Fällen unbestimmt ist (so wie die Reihenfolge des
Eintreffens von Teillösungen beim Problem unbestimmt ist), d.h.\ in
verschiedenen Programmläufen unterschiedlich sein kann.
...
Teilproblemen zu übertragenden Objekte müssen serialisierbar sein, d.h.\ das
Interface \texttt{java.io.Serializable} implementieren. Es ist zu beachten,
daß die Objekte bei der Übertragung tief kopiert werden, d.h.\ es werden
alle über Attribute direkt oder indirekt erreichbaren Objekte mitkopiert.
...
anderer Objekte) mehrfach übertragen wird, sind die Kopien des Objekts beim
...
\texttt{remotestore} ist aber zumindest sicher gestellt, daß ein Objekte,
