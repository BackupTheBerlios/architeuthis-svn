...
\item \texttt{receivePartialSolution}, die jeweils eine Teillösung übergibt
  und als Rückabewert die Gesamtlösung erwartet, falls diese schon erzeugt
...
abhängen oder konstant sein. Das Ausgeben der einzelnen Teilprobleme an
den Dispatcher und das Sammeln der Teillösungen werden von der
...
beiden Klassen eignen sich für Probleme, die am Anfang schon alle
Teilprobleme erstellen können und diese in einer durch Prioritäten
gekennzeichneten Reihenfolge berechnet werden sollen. Die Kennzeichnung
findet für jedes Teilproblem individuell statt, indem dessen Priorität durch
den Konstruktor gesetzt wird. Dabei bedeuten höhere Werte eine höhere
Priorität, d.h.\ dass dieses Teilproblem früher berechnet wird.
...
\texttt{PriorityPartialProblem} erben müssen.
...
\item \texttt{createPartialProblems}, die bei der ersten Anfrage für ein
  ...
  (deren Priorität gesetzt wurde) erstellen muss.

\item \texttt{createSolution}, die aufgerufen wird, wenn alle Lösungen
  eingegangen sind. Es muss auf jeden Fall eine Lösung geliefert werden,
